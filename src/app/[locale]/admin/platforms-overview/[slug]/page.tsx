...
export default function PlatformMonitoringPage() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      {tab === 'reports' && (
        <div className="grid lg:grid-cols-2 gap-5">
          <section className="rounded-2xl border border-neutral-200 bg-white p-5"><div className="flex justify-between gap-3"><h2 className="font-bold flex items-center gap-2"><FileText size={17} className="text-primary-600" /> التقارير</h2><Link href={`/${locale}/admin/impact?tab=reports&platformId=${platform.id}`} className="text-xs text-primary-700">إنشاء تقرير أثر</Link></div><div className="space-y-3 mt-4">{data.reports.map(report => <div key={report.id} className="rounded-xl border p-4 flex items-center justify-between gap-3"><div><div className="font-bold text-sm">{report.template.title}</div><div className="text-xs text-neutral-500 mt-1">{report.submittedBy || 'لم يحدد الرافع'} · {dateLabel(report.submittedAt || report.createdAt)}</div></div><StatusBadge status={report.status} /></div>)}</div></section>
